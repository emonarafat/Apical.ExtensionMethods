...
public static partial class Extensions
{
    ...
    {
        return Directory.EnumerateDirectories(@this.FullName).Select(x => new DirectoryInfo(x)).Where(x => predicate(x))
            .ToArray();
    }
    ...
    public static DirectoryInfo[] GetDirectoriesWhere(this DirectoryInfo @this, string[] searchPatterns,
        SearchOption searchOption, Func<DirectoryInfo, bool> predicate)
    {
        return searchPatterns.SelectMany(x => @this.GetDirectories(x, searchOption)).Distinct().Where(x => predicate(x))
            .ToArray();
    }
}